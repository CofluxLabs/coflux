...
export default function TargetsList({ projectId, environmentName, activeTarget }: Props) {
  const [repositories, _] = useTopic<Record<string, Record<string, models.Target>>>("projects", projectId, "environments", environmentName, "repositories");
  if (!repositories) {
    return <Loading />;
        ...
        {Object.entries(repositories).map(([repository, targets]) => (
          <Fragment key={repository}>
            <div className="flex items-center mt-4 py-1 px-2">
              <h2 className="flex-1 font-bold uppercase text-slate-400 text-sm">{repository}</h2>
            </div>
            {Object.keys(targets).length ? (
              <ul>
                {Object.entries(targets).map(([name, target]) => {
                  const isActive = !!activeTarget && activeTarget.repository == repository && activeTarget.target == name;
                  return (
                    <Target key={name} name={name} icon={IconSubtask} url={buildUrl(`/projects/${projectId}/tasks/${repository}/${name}`, { environment: environmentName })} isActive={isActive} />
                  );
                })}
              </ul>
            ) : (
