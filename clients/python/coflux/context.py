...
def set_execution(execution_id: str, client: client.Client, loop: asyncio.AbstractEventLoop) -> contextvars.Token:
...
def schedule_task(target: str, args: t.Tuple[t.Any, ...], repository: str | None = None) -> str:
    execution_id, client, loop = _get()
    task = client.schedule_task(execution_id, target, args, repository)
    return asyncio.run_coroutine_threadsafe(task, loop).result()

def schedule_step(target: str, args: t.Tuple[t.Any, ...], repository: str | None = None, cache_key: str | None = None) -> str:
    execution_id, client, loop = _get()
